...
export default function Auth()  { //the function component Auth is defined which is responsible for rendering the login form and handling user interaction
...
const onSubmit: SubmitHandler<LoginForm> = (data) => signInWithEmailAndPassword(data.email, data.password); // form submission
        ...
        <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
          ...
          <form className="space-y-4 md:space-y-6" onSubmit={handleSubmit(onSubmit)}>
            <div>
              ...
              <input type="email" {...register("email")} id="email" className="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 " placeholder="name@company.com" required />
              <p>{errors.email?.message}</p>
            </div>
            <div>
              <label htmlFor="password" className="block mb-2 text-sm font-medium text-gray-900 ">Password</label>
              <input type="password" {...register("password")} id="password" placeholder="••••••••" className="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" required />
